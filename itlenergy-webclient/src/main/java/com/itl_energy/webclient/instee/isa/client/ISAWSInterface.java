package com.itl_energy.webclient.instee.isa.client;
...
public class ISAWSInterface {
    ...
    public boolean beginSession(String user, String password) {
        ...
        String path = "/api/1.0/sessions";

        String[][] args = {{"Content-Type", "application/json"}, {"Accept", "application/json"}};

        String content = "{\"Login\":\"" + user + "\",\"Password\":\"" + password + "\"}";
        ...
        if (session != null) {
            ...
            this.duration = ((Long) session.get("Timeout")).intValue();
    ...
    public void listDevicesForHub() {
        String urlbase = "http://api.ienergy3.isa.pt:6969";
        String path = "/api/1.0/units";
        ...
        Map<?, ?> deviceList = ITLClientUtilities.parseJSONResponseToMap(response.getContent());
